...
mfig('Histogram FWI dataset'); clf;
for m = 1:n_att_fwi
...
mfig('Histogram dataset without area'); clf;
for m = 1:n_att_2
...
mfig('Boxplot Full Dataset without outliers from area attribute'); clf;
boxplot(M_data, attributeNames);
mfig('Boxplot for attribute area without outliers'); clf;
boxplot(area2);
...
rain2=M_data(:,12);
mfig('Boxplot and histogram Full Dataset and for attribute rain'); clf;
subplot(1,2,1);
boxplot(rain2);
subplot(1,2,2);
hist(rain2,100);
...
mfig('Boxplot Full Dataset (standardized) and boxplot and histogram for attribute rain without outliers'); clf;
subplot(1,3,1);
...
ffmc2=M_data(:,5);
mfig('Boxplot Full Dataset and histogram  for attribute FFMC'); clf;
subplot(1,2,1);
boxplot(ffmc2);
subplot(1,2,2);
hist(ffmc2);
...
mfig('Boxplot Full Dataset (standardized) and boxplot and histogram  for attribute FFMC  without outliers'); clf;
subplot(1,3,1);
boxplot(zscore(M_data), attributeNames);
subplot(1,3,2);
boxplot(ffmc3);
...
ISI2=M_data(:,8);
mfig('Boxplot  Full Dataset and boxplot histogram for attribute ISI'); clf;
subplot(1,2,1);
boxplot(ISI2);
subplot(1,2,2);
hist(ISI2);
...
mfig('Boxplot Full Dataset and boxplot and histogram for attribute ISI  without outliers'); clf;
subplot(1,3,1);
boxplot(zscore(M_data), attributeNames);
subplot(1,3,2);
boxplot(ISI3);
...
mfig('Histogram full dataset without ANY outlier'); clf;
for m = 1:n_att
    u = floor(sqrt(n_att)); v = ceil(n_att/u);
    subplot(u,v,m);
	hist(log(M_data(:,m)));
