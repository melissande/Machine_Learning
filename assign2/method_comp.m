...
for k1 = 1:K1 % For each crossvalidation fold
   ...
   y_test_est2 = knnclassify(X_test2, X_train2, y_train2, L, Distance);
    ...
    Error_in_gen(1)=sum(CV2.TestSize)*Error_Bayes/sum(CV1.TrainSize);
    Error_in_gen(2)=sum(CV2.TestSize)*Error_dt/sum(CV1.TrainSize);
    Error_in_gen(3)=sum(CV2.TestSize)*Error_kclass/sum(CV1.TrainSize);
...
fprintf('Error rate: %.1f%%\n', sum(CV1.TestSize)*Error_out_gen(:,2)/N);
