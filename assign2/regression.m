...
Error_gene_all = [Error_gene,Error_gene_feat,Error_gene_MET,Error_gene_FWI];
Error_train_all=[Error_trn,Error_train_feat,Error_train_MET,Error_train_FWI];
Model={'All data','Selec_feat','MET','FWI'};
mfig(['Error generalized and trained per model'])
subplot(1,2,1)
plot(1:4,Error_gene_all);
xlabel('Model','FontWeight','bold');
ylabel('Error');
title('Generalized error');
axis([1 4 4000  4800])
subplot(1,2,2)
plot(1:4,Error_train_all)
xlabel('Model','FontWeight','bold');
ylabel('Error');
axis([1 4 4000  4800])
title('Trained error');